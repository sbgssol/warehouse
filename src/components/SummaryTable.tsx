...
export default function SummaryTable(props: { data: ImportData.Data; input_ref: MutableRefObject<HTMLInputElement[]> }) {
        ...
        <table className="text-center mt-2">
          ...
          <tbody className="">
            {props.data.danh_sach_san_pham.flatMap((detail, index) => (
              <tr key={index}>
                <td className="text-left border border-green-600 overflow-hidden max-w-[200px] pl-1" width={200}>
                  {detail.ma_hang}
                </td>
                <td className="text-left border  border-green-600 overflow-hidden max-w-[500px] pl-1" width={500}>
                  {detail.ten_hang}
                </td>
